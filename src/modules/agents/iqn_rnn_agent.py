...
class IQNRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.phi = nn.Linear(args.quantile_embed_dim, args.hidden_dim)
    ...
    def forward(self, inputs, hidden_state, forward_type=None):
        ...
        shape = h.shape
        batch_size = shape[0]
        h2 = h.reshape(batch_size, 1, self.args.hidden_dim).expand(-1, n_rnd_quantiles, -1).reshape(-1, self.args.hidden_dim)
        assert h2.shape == (batch_size * n_rnd_quantiles, self.args.hidden_dim)
        shape = h2.shape
        # Generate random quantiles
        if self.args.name == "diql":
            rnd_quantiles = th.rand(batch_size * n_rnd_quantiles).to(self.device)
            ...
            rnd_quantiles = th.rand(batch_size_grouped, 1, n_rnd_quantiles).to(self.device)
        ...
        feature_id = th.arange(0, self.quantile_embed_dim).to(self.device)
        ...
        assert q_phi.shape == (batch_size_grouped * n_rnd_quantiles, self.args.hidden_dim)
        if self.args.name != "diql":
            q_phi = q_phi.view(batch_size_grouped, n_rnd_quantiles, self.args.hidden_dim)
            q_phi = q_phi.unsqueeze(1).expand(-1, self.args.n_agents, -1, -1).contiguous().view(-1, self.args.hidden_dim)
        assert q_phi.shape == (batch_size * n_rnd_quantiles, self.args.hidden_dim)
